...
| Metric | Value |
|---|---|
| **Setup WNS** | **0.258871 ns** |
| Setup TNS | 0 |
| Hold WNS | −0.0126 ns |
| Hold TNS | −0.0404 ns |
| **Core Area** | **4,306,330 μm²** |
| Die Area | 4,392,690 μm² |
| Instance Area | 619,450 μm² |
| Total Power | 0.199 W |
| Fmax | 267.30 MHz |
| Proxy cost (NG45) | 0.7207 |
| Setup violations | 0 |
| Hold violations | 4 |

Source: `output/orfs_eval_v6/` — full ORFS flow (synthesis → floorplan → PDN →
placement → CTS → routing → STA) on NG45 ariane133, generated by the v1.0
placer plus the spacing-polish + manufacturing-grid-snap fixes documented in
this commit. The placer's narrow-channel-aware spacing polish and a 5 nm grid
snap in `scripts/generate_macro_placement_tcl.py` were required to make ORFS
PDN and detailed routing complete; without them PDN-0179 ("Unable to repair
all channels") and DRT-0416 ("off-grid pin shape") fire and the flow
terminates before STA.
