...
def _load_placer(path: Path):
    ...
    if spec := importlib.util.spec_from_file_location(path.stem, str(path)):
        ...
        # Register in sys.modules BEFORE exec_module so that @dataclass and
        # multiprocessing.spawn pickling can look the module up by name.
        # Per Python docs (importlib "Importing a source file directly"):
        # without this, dataclass(frozen=True) and ProcessPoolExecutor break.
